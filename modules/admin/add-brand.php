...
if (isset($_POST['submit_add_brand'])) {
    ...
    $exists = selectOne("SELECT ID FROM brand WHERE name = ?", [$name]);
    if ($exists) {
        $errors[] = "Tên thương hiệu \"$name\" đã tồn tại!";
    }
    ...
    if (empty($errors)) {
        ...
        $newBrand = insert('brand', $brandData);

        if ($newBrand) {
            echo "<script>
                    alert('Đã thêm thương hiệu \"$name\" thành công!');  
                    window.location.href = '" . _HOST_URL . "?module=admin&action=add-brand';          
                  </script>";
            exit();
        } else {
    ...
    <main class="app-main">
        ...
        <div class="app-content">
            ...
            <div class="container-fluid">
                ...
                <div class="row mt-2">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Thêm thương hiệu (Brand) </h3>
                    </div>
                    <div class="col-sm-6">
                ...
                <div class="add-product">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <button type="submit" name="submit_add_brand" class="btn btn-primary">Thêm thương hiệu </button>
                    </form>
                </div>
            </div>
            <!--end::Container-->
